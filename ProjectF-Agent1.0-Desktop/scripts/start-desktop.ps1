...
$expectedBuildVersion = "desktop-1.4.19"
...
function Initialize-FuraPython {
    ...
    if ($installedHash -eq $requiredHash) {
        & $pythonPath -c "import fastapi, uvicorn, httpx, pydantic, yaml, PIL, jsonschema, multipart, imageio_ffmpeg" 2>$null
...
function Test-FuraReady {
    ...
    return $null -ne $health -and $health.status -eq "ready" -and $health.build_version -eq $expectedBuildVersion
}
...
function Stop-StaleFuraServer {
    $health = Get-FuraHealth
    $looksLikeFura = $null -ne $health -and $health.status -eq "ready" -and $health.skills -eq 7 -and $health.routes -eq 6
    if (-not $looksLikeFura -or $health.build_version -eq $expectedBuildVersion) {
        return
    }
    $ownerPid = Get-FuraPortOwner
    if ($ownerPid) {
        Write-Host "[FURA] Replacing stale desktop service ($($health.build_version))..."
