...
class OpenAICompatibleVisionAdapter:
    ...
    async def analyze(self, skill: SkillDefinition, media: MediaArtifact, pet: PetContext) -> dict[str, Any]:
        ...
        if isinstance(message_content, list):
            message_content = "".join(str(item.get("text", "")) for item in message_content)
    ...
    async def repair_result(
        ...
        if not choices:
            raise ValueError("修复模型响应缺少 choices")
...
class QwenNativeVideoAdapter:
    ...
    def fps_for_skill(skill_name: str) -> float:
        configured = os.getenv("AGENT_VIDEO_FPS")
        if configured:
            fps = float(configured)
        else:
            fps = 6.0 if skill_name == "home-health-check-gait" else 4.0
    ...
    async def analyze(self, skill: SkillDefinition, media: MediaArtifact, pet: PetContext) -> dict[str, Any]:
        ...
        if not choices:
            raise ValueError("原生视频模型响应缺少 choices")
