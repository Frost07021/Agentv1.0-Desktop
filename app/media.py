...
class MediaProcessor:
    ...
    def _prepare_video(self, path: Path) -> MediaArtifact:
        ...
        rotation = self._display_rotation(probe)
    ...
    def normalize_video_orientation(self, media: MediaArtifact, target_bytes: int | None = None) -> Path:
        """Return a cached video whose pixels already match the display orientation.

        Phone videos often store landscape H.264 pixels plus a QuickTime display
        matrix. Image extraction honors that matrix, while some native multimodal
        endpoints do not. Transcoding only those rotated files keeps both analysis
        channels on the same upright pixels without modifying the uploaded source.
        """
        if media.type != "video":
            raise MediaError("MEDIA_TYPE_MISMATCH", "只有视频可以执行方向标准化")
        if (
            round(abs(media.rotation_degrees)) % 360 == 0
            and (target_bytes is None or media.path.stat().st_size <= target_bytes)
        ):
            return media.path

        stat = media.path.stat()
        proxy_profile = f"upright-h264-fast-crf20-v3-target-{target_bytes or 0}"
        fingerprint = (
            f"{media.path.resolve()}:{stat.st_size}:{stat.st_mtime_ns}:"
            f"{media.rotation_degrees:.3f}:{proxy_profile}"
        )
        digest = hashlib.sha1(fingerprint.encode("utf-8")).hexdigest()[:12]
        output_dir = self.runtime_dir / "normalized_videos"
        output_dir.mkdir(parents=True, exist_ok=True)
        output = output_dir / f"{media.path.stem}-{digest}-upright.mp4"
        ...
        if target_bytes is None:
            encoding_profiles = [["-crf", "20"]]
        else:
            duration = float(media.duration or 0.0)
            if target_bytes <= 0 or duration <= 0:
                raise MediaError("MEDIA_ORIENTATION_NORMALIZATION_ERROR", "方向标准化压缩参数无效")
            base_bitrate = max(500_000, int((target_bytes * 8 / duration) * 0.88))
            encoding_profiles = []
            for ratio in (1.0, 0.78):
                bitrate = max(500_000, int(base_bitrate * ratio))
                encoding_profiles.append(
        ...
        for encoding_profile in encoding_profiles:
            ...
            if (
                ...
                and (target_bytes is None or output.stat().st_size <= target_bytes)
            ):
                return output

        if result is None or result.returncode != 0 or not output.is_file() or output.stat().st_size <= 0 or (
            target_bytes is not None and output.stat().st_size > target_bytes
        ):
            output.unlink(missing_ok=True)
            detail = result.stderr.decode("utf-8", errors="replace").strip().splitlines() if result else []
            message = detail[-1][:160] if detail else "未知转码错误"
            raise MediaError("MEDIA_ORIENTATION_NORMALIZATION_ERROR", f"视频方向标准化失败: {message}")
        return output
    ...
    def _prepare_pdf(self, path: Path) -> MediaArtifact:
        stat = path.stat()
        fingerprint = f"{path.resolve()}:{stat.st_size}:{stat.st_mtime_ns}:pdf-200dpi-v2"
        digest = hashlib.sha1(fingerprint.encode("utf-8")).hexdigest()[:10]
        page_dir = self.runtime_dir / "pdf_pages" / f"{path.stem}-{digest}"
        page_dir.mkdir(parents=True, exist_ok=True)
        cached_pages = sorted(page_dir.glob("page-*.jpg"))
        if cached_pages:
            with Image.open(cached_pages[0]) as image:
                width, height = image.size
            return MediaArtifact(
                "pdf", path, "application/pdf", stat.st_size, width, height, keyframes=cached_pages
            )
        ...
        try:
            ...
            try:
                ...
                for index in range(len(document)):
                    page = document[index]
                    try:
                        bitmap = page.render(scale=200 / 72, draw_annots=True)
                        try:
                            image = bitmap.to_pil().convert("RGB")
                            output = page_dir / f"page-{index + 1}.jpg"
                ...
                document.close()
            if not pages:
                raise ValueError("PDF 没有生成页面图片")
            with Image.open(pages[0]) as image:
                width, height = image.size
            return MediaArtifact(
                "pdf", path, "application/pdf", stat.st_size, width, height, keyframes=pages
            )
        ...
        # Compatibility fallback for environments that already provide Poppler.
        converter = shutil.which("pdftoppm")
        if not converter:
            detail = f": {pdfium_error}" if pdfium_error else ""
            raise MediaError(
                "PDF_CONVERTER_MISSING",
                f"PDF 渲染组件不可用，请重新安装 requirements.txt{detail}",
            )
        ...
        with Image.open(pages[0]) as image:
            width, height = image.size
        return MediaArtifact("pdf", path, "application/pdf", stat.st_size, width, height, keyframes=pages)
    ...
    @staticmethod
    def _display_rotation(probe: str) -> float:
        matrix = re.search(r"rotation of\s+(-?\d+(?:\.\d+)?)\s+degrees", probe, re.IGNORECASE)
        if matrix:
            return float(matrix.group(1))
        metadata = re.search(r"(?:^|\n)\s*rotate\s*:\s*(-?\d+(?:\.\d+)?)", probe, re.IGNORECASE)
        return float(metadata.group(1)) if metadata else 0.0
