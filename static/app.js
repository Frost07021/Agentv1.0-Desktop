...
function renderAnalysisError(error, scope, name) {
  ...
  $("#result-content").innerHTML = `
    <div class="result-wrap result-error-wrap">
      <div class="result-head"><div><span class="feature-kicker">ANALYSIS INTERRUPTED</span><h2>分析未完成</h2><p>${escapeHTML(name)} · 当前素材已保留</p></div><button class="result-close" aria-label="关闭">×</button></div>
      <div class="severity-card result-error-card"><span class="severity high">需重试</span><p>${escapeHTML(message)}</p></div>
      <section class="result-section"><h3>下一步</h3><p class="result-error-help">请检查网络后重新分析。若再次失败，错误会继续显示在这里，不会静默关闭。</p></section>
...
function renderResult(result, scope, resultId) {
  ...
  const degradedVideo = scope !== "report" && runtime.analysis_quality === "degraded_dense_storyboard";
  ...
  $("#result-content").innerHTML = `
    <div class="result-wrap">
      ...
      ${qualityNotice}
      <div class="severity-card"><span class="severity ${severityClass(summary.severity)}">${escapeHTML(summary.severity || "已完成")}</span><p>${escapeHTML(summary.summary || "分析已完成。")}</p></div>
